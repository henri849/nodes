    ...
    def publisher(self):
        ...
        while True:
            if self.ser.in_waiting >0: # if the radio has received anything it will go into the radio's buffer, this checks if the buffer is not empty
                inst.data = self.parse(self.ser.readline())# readline waits for a newline character "\n" then returns everythign before it including the \n 
                self.get_logger().debug('radio: ' + str(inst.data))
                self.publisher_.publish(inst)

    def parse(self,msg):
        msg = msg.split(b',')   
        return list(map(lambda a: float(a),msg))
